...
def specpolfinalstokes(infile_list,propid,logfile):
    ...
    wav_l,heff_l,hpa_l,qeff_l = np.loadtxt(datadir+'polcal.txt',dtype=float,unpack=True)
    ...
    # organize data using names
        ...
        for i in range(files):
            ...
            rawlist.append([i,object,config,wvplt,count])
        for col in (4,3,2,1): rawlist = sorted(rawlist,key=operator.itemgetter(col))
    ...
    # for each obs (object/ config), combine stokes, apply efficiency and PA calibration as appropriate for pattern
        obss = len(obslist)
        for obs in range(obss):
            ...
            if wppat.count('Linear'):
                ...
                if wppat=='Linear':
                    stokes_fw[1:] = stokes_ksw[k:k+2,1]*(stokes_fw[0]/stokes_ksw[k:k+2,0])
                    var_fw[1:] = var_ksw[k:k+2,1]*(stokes_fw[0]/stokes_ksw[k:k+2,0])**2
